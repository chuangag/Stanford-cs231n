...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    exp_scores_denomed=exp_scores/denom.reshape(-1,1)
    loss=np.sum(-np.log(exp_scores_denomed[range(N),list(y)]))
    loss/= N
    ...
    #############################################################################
    y_pred=exp_scores_denomed.copy()
    y_pred[range(N),y]-=1 # y_pred - y 
    
    grads['b2']=np.sum(y_pred,axis=0)/N
    grads['W2']=np.dot(hidden_layer.T,y_pred/N)+2*reg*W2

    dz1=(y_pred/N).dot(W2.T) # (N,H)=(N,C)*(C,H)
    dRelu=(hidden_layer>0)*dz1 # only propagate activated neurons
